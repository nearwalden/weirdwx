...
// v3
const api_key = "AAElAAAAAAAAAD06GJUTsLNbCbvZQCnV8ksyS4qq5tENyS4AGRwYqbMr";
...
const darkskyVersionValid = ["v2", "v3", "v3bulk"];

var darkskyVersion = "v3bulk";

function wxSummary (lat, lon, v, callback)
{
	if (darkskyVersionValid.includes(v)) {
		darkskyVersion = v;
	}
	else {
		callback ("bad version: " + v, null);
		return;
	}
...
function collectHistorical (client, opts, res, callback) 
{
	if (darkskyVersion == "v2") {
		...
		collectHistoricalV3Bulk (client, 0, opts, res, callback);
			}
	else {
...
function collectHistoricalV3 (client, count, opts, res, callback) {
	...
	// loc
	var loc = [[opts.lat, opts.lon].toString()]
	// console.log("Processing year" + count+1);
...
const bulkLimits = {days: 20}

function collectHistoricalV3Bulk (client, count, opts, res, callback) {
	var bl = bulkLimits['days'];
	var countLeft = years - count;
	// subtract one extra since have data for now
	var year = opts.date.getFullYear() - 1;
	// array to collect into
	var days = [];
	// don't change the original
	var d2 = new Date(opts.date);
	for (var i = 0; i < Math.min(countLeft, bl); i++) {
		d2.setFullYear(year - count - i);
		days[i] = d2.toISOString().split("T")[0];
	}
	// loc
	var loc = [[opts.lat, opts.lon].toString()]
	// console.log("Processing year" + count+1);
			...
			if (err) {
				...
				for (var i=0; i < histFieldsV3.length; i++) {
					...
					for (var j=0; j < data.data.length; j++) {
						res.hist[field][j + count] = data.data[j].day[field];
					}
				}
				if (countLeft <= bl) {
					// use v3 summarize, no difference
					summarizeV3(res, callback);
				}
				else {
					collectHistoricalV3Bulk (client, count+bl, opts, res, callback);
				}
			}
		});	
